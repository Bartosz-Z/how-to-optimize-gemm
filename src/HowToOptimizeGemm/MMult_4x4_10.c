...
void AddDot4x4( int k, double *a, int lda,  double *b, int ldb, double *c, int ldc )
{
  ...
  for ( p=0; p<k; p++ ){
      a_0p_a_3p_vreg.v = _mm256_loadu_pd( (double *) &A( 0, p ) );

    b_p0_vreg.v = _mm256_broadcast_sd( (double *) b_p0_pntr++ );   /* load and duplicate */
    b_p1_vreg.v = _mm256_broadcast_sd( (double *) b_p1_pntr++ );   /* load and duplicate */
    b_p2_vreg.v = _mm256_broadcast_sd( (double *) b_p2_pntr++ );   /* load and duplicate */
    b_p3_vreg.v = _mm256_broadcast_sd( (double *) b_p3_pntr++ );   /* load and duplicate */
